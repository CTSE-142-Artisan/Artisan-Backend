...
class AuthSecurityMvcTest {
    ...
    @Test
    void validateSeller_shouldAllowAnonymousGet() throws Exception {
        when(userService.validateSeller("seller-1")).thenReturn(true);

        mockMvc.perform(get("/api/users/seller-1/validate-seller"))
                .andExpect(status().isOk())
